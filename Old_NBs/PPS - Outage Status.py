...
# MAGIC
# MAGIC ## Purpose
# MAGIC To create a timeline of outage status per meter and job by applying business rules that mirror the PPS outage system to analyze the number of meters in "Don't Know" status throughout an outage job
# MAGIC
# MAGIC ## Deliverable
...
interval_data = [{"job_interval": "0 - 1 min", "job_time_step": 0, "job_end_time_step": 60},
...
job_timeline = full_business_95 \
  ...
  .filter(col("interval_from_job_start") >= 0)
...
cond = (col("interval_from_job_start") >= col("job_time_step")) & \
